system("nard2 new $(dirname $(whereis nard2 | awk '{print $2}'))/src/user_functions/heat_NA_w_reaction.f90 _convtesting_CN_2");

dt_range = 2.^(-4:-1:-9);
foldername = sprintf("%s/test_CN_2", pwd());
mkdir(foldername);
if false
parfor i = 1:numel(dt_range)
    ...
    nard2('_convtesting_CN_2', conf_filename);
end
end
errors = zeros(size(dt_range));
for i = 1:length(dt_range)
    
    savefilename = sprintf("%s/test_CN_2/results_%i_CN_2.h5",pwd(), i);
